...
impl<'a> UiContext<'a> {
    pub fn push_quad(&mut self, x: f32, y: f32, w: f32, h: f32, color: [f32; 4]) {
        push_quad_raw(self.vertices, self.indices, self.white_uv, x, y, w, h, color);
    }
    ...
    pub fn push_char(
        ...
        if let Some(info) = self.atlas.get_or_rasterize(self.queue, c, font_size) {
            ...
            if is_powerline {
                let sliver_w = 1.5;
                if cp % 4 == 0 {
                    push_quad_raw(self.vertices, self.indices, white_uv, x, y, sliver_w, h, color);
                } else if cp % 4 == 2 {
                    push_quad_raw(self.vertices, self.indices, white_uv, x + w - sliver_w, y, sliver_w, h, color);
                }
            }
    ...
    pub fn push_icon(&mut self, icon_path: &str, x: f32, y: f32, color: [f32; 4], size: f32) -> f32 {
        if let Some(info) = self.atlas.get_or_rasterize_icon(self.queue, icon_path, size) {
            let start = self.vertices.len() as u16;
            let rx = x.round();
            let ry = y.round();
    ...
    pub fn push_str_spaced(&mut self, text: &str, mut x: f32, y: f32, color: [f32; 4], font_size: f32, spacing: f32) -> f32 {
        let start_x = x;
        for c in text.chars() {
